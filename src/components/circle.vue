<template>
  <div>
    ...
    <el-row style="padding:10px" type="flex">
      <el-col :span="8">
          <el-image style="width:80px;height:80px;border-radius:100%"></el-image>
      </el-col>
    ...
    <el-row style="padding:6px 14px;border-bottom:6px #00000010 solid" v-for="(item,index) in task">
      ...
      <el-col :span="24"><el-button size="mini" style="float:right" class="clearfix" @click="accept(item.id,index)" :type="flagStatus(item.status)">{{item.status?'已接受':'接受任务'}}</el-button></el-col>
    </el-row>
  </div>
</template>
